            ...
            if st.button("🔍 구간 매칭 및 비교 분석", type="primary"):
                if reference_df is not None:
                    with st.spinner("데이터 분석 중..."):
                        try:
                            # 구간 매칭
                            matched_reference, user_data, matching_info = find_matching_range(reference_df, user_df,
                                                                                              column_mapping)
                            ...
                            if not matched_reference.empty:
                                ...
                                if production_col in matched_reference.columns:
                                    try:
                                        # 생산량 컬럼을 숫자로 변환
                                        production_data = pd.to_numeric(matched_reference[production_col],
                                                                        errors='coerce')
                                ...
                                if max_production_idx is not None and max_production_value is not None:
                                    ...
                                    st.info(
                                        f"🏆 사용자의 누적일사량, 외기기온 구간의 조건에서 최대 **{max_production_value:.1f}㎏/3.3㎡** 수확 가능")

                                # 환경 데이터 비교 분석 (최대 생산량 행 기준)
                                max_production_row_data = None
                                if max_production_idx is not None:
                                    max_production_row_data = matched_reference.loc[max_production_idx]
                                env_gauge_figs, env_valid_comparisons, _, _ = create_comparison_charts(
                                    matched_reference, user_data, column_mapping, max_production_row_data)
                                ...
                                if env_gauge_figs:
                                    ...
                                    use_single_column = len(env_gauge_figs) <= 3 or st.sidebar.button("📱 모바일 모드",
                                                                                                      help="차트를 세로로 배치합니다")
